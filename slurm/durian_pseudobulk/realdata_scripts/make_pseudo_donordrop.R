...
subsetcelltypes=Sys.getenv("SUBSETCELLTYPES")
...
################################################################
# get donor-specific dropout
################################################################
donors = sort(unique(testing_phenoData$sampleID))
for(i in 1:length(donors)){
  print(paste0("downsampling donor ",i))
  ids = testing_phenoData$cellID[which(testing_phenoData$sampleID == donors[i])]
  p_donor = exp(-lambda*rowMeans(data[,ids])^2)
  for(j in ids){
    dropgenes = names(which(rbernoulli(n=1,p=p_donor)))
