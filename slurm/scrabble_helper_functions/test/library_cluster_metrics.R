run_dunn <- function(
    ...
    for(prefixind in 1:length(prefixes)){
        ...
        imethods = list.files(file.path(outputdirs[fileind],"output_fit"))
        for(i in 1:length(imethods)){
            imputedir = file.path(outputdirs[fileind],"output_fit",imethods[i])
            if(file.exists(file.path(imputedir,"imputed_C.csv"))){
                imputedC = read.csv(file.path(imputedir,"imputed_C.csv"),row.names=1)
                pDataC = read.csv(file.path(sourcepath,paste0(prefix[1],"_pDataC.csv")),row.names=1)
                pDataC = filter(pDataC,cellType %in% typekeep)
                common_cells = intersect(colnames(imputedC),rownames(pDataC))

                pDataC = pDataC[common_cells,]
                imputedC = imputedC[,common_cells]

                methodname = strsplit(imethods[i],"_")[[1]][2]

                ##########################################
                # get the dunn index (cluster quality)
                ##########################################
                set.seed(42)
                tsne = Rtsne::Rtsne(t(imputedC))
                cstats = fpc::cluster.stats(dist(tsne$Y),as.integer(as.factor(pDataC$cellType)))

                if(!runknn){
                    df = data.frame(
                        metric=c("MinInterMaxIntra","MinMeanInterMaxMeanIntra"),
                        x=c(cstats$dunn,cstats$dunn2),
                        celltype=c(NA,NA),
                        method=c(methodname,methodname),
                        dataset=c(prefix[1],prefix[1]))
                }else{
                    tdata = as.data.frame(cbind(tsne$Y,as.factor(pDataC$cellType)))
                    colnames(tdata) = c("y1","y2","cellType")
                    knn5 = neighbr::knn(tdata,tdata[,1:2],categorical_target="cellType",comparison_measure="squared_euclidean",k=5)

                    cstatsknn = fpc::cluster.stats(dist(tsne$Y),as.numeric(knn5$test_set_scores[,1]))

                    df = data.frame(
                        metric=c("MinInterMaxIntraLab","MinMeanInterMaxMeanIntraLab","MinInterMaxIntraKNN","MinMeanInterMaxMeanIntraKNN"),
                        x=c(cstats$dunn,cstats$dunn2,cstatsknn$dunn,cstatsknn$dunn2),
                        celltype=c(NA,NA,NA,NA),
                        method=c(methodname,methodname,methodname,methodname),
                        dataset=c(prefix[1],prefix[1],prefix[1],prefix[1]))
                }

                ##########################################
                # transfer both metrics to master list
                ##########################################
                print("saving metrics")
                slotname = paste(prefix[1],methodname,sep='.',collapse="")
                master.orig[[slotname]] = df
            }
        }
    }

    if(length(master.orig)>0){
        df2 = do.call(rbind,master.orig)
        rownames(df2) = 1:nrow(df2)
        return(df2)
    }
}

run_cluster_metrics <- function(
                        savepath_master,
                        outputmaster,
                        sourcepath,
                        prefixes,
    ...
    for(prefixind in 1:length(prefixes)){
        ...
        for(i in 1:length(imethods)){
            ...
            if(file.exists(file.path(imputedir,"imputed_C.csv"))){
                ...
                ##########################################
                # get the pgcs (pathway gene correlation score)
                ##########################################

                allsets0 = list()
                for(j in 1:length(genesets)){
                    common_genes = intersect(genesets[j][[1]],rownames(imputedC))
                    if(length(common_genes)>0){
                        corr_j = get_pathway_corr(common_genes,imputedC)
                        allsets0[[names(genesets[j])]] = corr_j[1]/corr_j[2]-1
                    }else{
                        print("no genes present")
                    }
                }

                allsets = unlist(allsets0)
                allsets = allsets[!is.nan(allsets)]
                df = data.frame(
                    metric=rep("dPGCS",length(allsets)),
                    x=allsets,
                    pathname=names(allsets),
                    method=rep(methodname,length(allsets)),
                    dataset=rep(prefix[1],length(allsets)))
    ...
    if(length(master.orig)>0){
        dir.create(savepath_master,recursive=TRUE)
        df2 = do.call(rbind,master.orig)
        rownames(df2) = 1:nrow(df2)
        write.csv(df2,file=file.path(savepath_master,"metrics.csv"))
        return(df2)
    }
}
...
plot_clustermetrics <- function(df_orig,outputdir,yvars = c("dPGCS")){
    ...
    for(j in 1:length(yvars)){
        ...
        ylab(yvars[j])
        ggsave(plot=p,file=file.path(outputdir,paste0("metrics_",yvars[j],".svg")),width=100,height=30,units="cm")
        plist[[yvars[j]]] = p
    }
    return(plist)
}
...
run_clusterMetrics_final_nested_real <- function(
    ...
    metrics_df0$modelname = metrics_df0$imputemodel
    ...
    fn = file.path(savepath_master,subdir,"mean_plot_point.svg")
    ...
    for(iii in 1:length(metrics_prefix)){
        ...
        fn = file.path(savepath_master,subdir,"scatter",paste0(metrics_prefix[iii],"_cell.svg"))
        p = ggplot(
            data=metrics_df0,
            ...
            # ggsave(plot=p,file=file.path(savepath_master,paste0("ds_",dsname,",nIn_",nIn,",nOut_",nOut,",nSDC_",nSDC,"_conv.svg")),width=plotwidth,height=plotheight)        
            ggsave(plot=p,file=fn,width=plotwidth,height=plotheight)

        print(paste0("plotting scatter prefix:",metrics_prefix[iii],", gene"))
        fn = file.path(savepath_master,subdir,"scatter",paste0(metrics_prefix[iii],"_gene.svg"))
        p = ggplot(
            data=metrics_df0,
        ...
        fn = file.path(savepath_master,subdir,"box",paste0(metrics_prefix[iii],"_cell.svg"))
        p = ggplot(
            data=metrics_df0,
        ...
        fn = file.path(savepath_master,subdir,"box",paste0(metrics_prefix[iii],"_gene.svg"))
        p = ggplot(
            data=metrics_df0,
        ...
        fn = file.path(savepath_master,subdir,"violin",paste0(metrics_prefix[iii],"_cell.svg"))
        p = ggplot(
            data=metrics_df0,
        ...
        fn = file.path(savepath_master,subdir,"violin",paste0(metrics_prefix[iii],"_gene.svg"))
        p = ggplot(
            data=metrics_df0,
        ...
        fn = file.path(savepath_master,subdir,"bar",paste0("max_",metrics_prefix[iii],"_cell.svg"))
        p = ggplot(
            data=metrics_df0,
            aes(x=modelname,y=get(yvar_cell),color=modelname)) + 
        ...
        fn = file.path(savepath_master,subdir,"bar",paste0("max_",metrics_prefix[iii],"_gene.svg"))
        p = ggplot(
            data=metrics_df0,
            aes(x=modelname,y=get(yvar_gene),color=modelname)) + 
            geom_bar(stat="identity") +
...
run_clusterMetrics_final_nested_sim <- function(
    ...
    for(iii in 1:length(metrics_prefix)){
        ...
        p = ggplot(
            ...
            print(paste0("plotting bar prefix:",metrics_prefix[iii],", cell"))
            fn = file.path(savepath_master,subdir,"bar",paste0("mean_",metrics_prefix[iii],"_cell.svg"))
            p = ggplot(
                data=metrics_df0) + 
                geom_bar(aes(x=modelname,y=get(yvar_cell),fill=modelname),position = "dodge", stat = "summary", fun.y = "mean") +
                rotate_x_text(45) +
                facet_grid(rows=vars(mean_dropout_sparsity)) +
            theme(axis.text.x = element_blank()) +
                ggtitle(paste0("max_cell ",metrics_prefix[iii]))
            ...
            print(paste0("plotting bar prefix:",metrics_prefix[iii],", gene"))
            fn = file.path(savepath_master,subdir,"bar",paste0("mean_",metrics_prefix[iii],"_gene.svg"))
            p = ggplot(
                data=metrics_df0) + 
                geom_bar(aes(x=modelname,y=get(yvar_gene),fill=modelname),position = "dodge", stat = "summary", fun.y = "mean") +
                rotate_x_text(45) +
                facet_grid(rows=vars(mean_dropout_sparsity)) +
            theme(axis.text.x = element_blank()) +
                ggtitle(paste0("max_gene ",metrics_prefix[iii]))
