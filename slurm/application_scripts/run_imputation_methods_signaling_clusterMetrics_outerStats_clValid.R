...
if(imethod=="dropout"){
    ...
    cstats = run_clvalid(impdata=impresult,runstability = runstability)
    logdf <- data.frame(
    ...
    run_cluster_plots(impresult,pDataC,file.path(savepath,"cluster_plots"))
    ...
    cstats = run_clvalid(impdata=C,runstability = runstability)
    logdf0 <- data.frame(
        iter = as.integer(c(NA)),
        ldaMeanRhat = as.numeric(c(NA)),
        scrabbleLoss = as.numeric(c(NA)),
